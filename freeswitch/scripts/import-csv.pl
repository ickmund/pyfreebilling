...
my $update_str = $dbh->prepare("UPDATE company SET supplier_balance = supplier_balance - ? WHERE id = ?");
...
#                print " cost_rate : $fields[15] \n\n";
            ...
            if ($companyid != 0){
                #my $update_str = $dbh->prepare("UPDATE company SET supplier_balance = supplier_balance - ? WHERE id = ?");
                $update_str->bind_param(1, $totalcost, SQL_FLOAT) or die $DBI::errstr;
                $update_str->bind_param(2, $companyid, SQL_INTEGER) or die $DBI::errstr;
                #print " company : $companyid \n\n";
                #print $dbh->{Statement}."\n";
                $update_str->execute or die $DBI::errstr;
                $update_str->finish;
